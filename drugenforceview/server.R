...
getreportid <- reactive({
  mydf <- getquery()
  tmp <- mydf$df.results
  id <- tmp$recall_number
  if (is.null(id)){
    id = 'Missing Recall Number'
  }
  return(id)
})
...
getfullquery <- reactive({
  ...
  myurl <- buildURL(v1, t1, limit=1, db='/drug/', type='enforcement')
...
getquery <- reactive({
  ...
  myurl <- buildURL(v1, t1,  limit=1, skip=getskip(), db='/drug/', type='enforcement')
...
output$table1title <- renderText({   
  mydf2 <- getquery()
  s  <- paste('<h4>Recall Number=', getreportid(), '</h4>') 
  return( s )
})
...
output$querytitle <- renderText({ 
  mydf2 <- getquery()
  return( paste('<h4>Recall Number =', getreportid(), '</h4>',
...
output$reportid <- renderUI({
  mydf2 <- getquery()
  p( paste('Recall Number=', getreportid() ) )
})
