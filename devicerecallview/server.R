...
getfullquery <- reactive({
  ...
  myurl <- buildURL(v1, t1, limit=1, db='/device/', type='recall')
...
getquery <- reactive({
  ...
  myurl <- buildURL(v1, t1,  limit=1, skip=getskip(), db='/device/', type='recall')
...
output$table1title <- renderText({   
  mydf2 <- getquery()
  s  <- paste('<h4>Event ID =', getreportid(),
              '<br><br>Recall Fields', 
              '</h4>') 
  return( s )
})
