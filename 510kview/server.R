...
getfullquery <- reactive({
  ...
  myurl <- buildURL(v1, t1, limit=1, db='/device/', type='510K')
...
getquery <- reactive({
  ...
  myurl <- buildURL(v1, t1,  limit=1, skip=getskip(), db='/device/', type='510K')
  myurl <- gsub('Any Variable:', "", myurl, fixed=TRUE)
  mydf <- fda_fetch_p(session, myurl)
  if ( !length(mydf)==0  )
    {
      tmp <- mydf$results
    mydf$results <- data.frame( time_to_approval=as.Date(tmp[,'decision_date'])- 
      as.Date( tmp[,'date_received']), tmp )
    } 
  #   print('url')
  out <- c(df=mydf, url=myurl )
  return(out)
...
output$openfdatabletitle <- renderText({  
  mydf2 <- getquery()
  s  <- paste('<h4>K Number=', getreportid(), '</h4>',
              '<br>Days to approve =', mydf2$df.results$time_to_approval ,
              '<br><br><h4>OpenFDA variable</h4>') 
  return( s )
})

output$openfda <- renderTable({ 
  mydf <- getquery()
  mydf <- mydf$df.results$openfda
#   tmp <- mydf
...
output$user_dmtabletitle <- renderText({   
  ...
  s  <- paste('<h4>K Number=', getreportid(), '</h4>',
            '<br>Days to approve =', mydf2$df.results$time_to_approval ,
            '<br><br><h4>Device Manufacturer</h4>') 
  return( s )
})
...
output$user_dm <- renderTable({  
  ...
  # browser()
  myvars <- getapplicantvars()
  return(extractcols(mydf, myvars))
})

#Other**************************
output$devicetabletitle <- renderText({  
  mydf2 <- getquery()
  s  <- paste('<h4>K Number=', getreportid(), '</h4>',
              '<br>Days to approve =', mydf2$df.results$time_to_approval ,
              '<br><br><h4>mydf2Patient</h4>') 
  return( s )
})

output$device <- renderTable({  
  mydf <- getquery()$df.results
  myvars <- get510devicevars()
  return(extractcols(mydf, myvars))
})
...
output$querytitle <- renderText({ 
  mydf2 <- getquery()
  return( paste('<h4>K Number =', getreportid(), '</h4>',
#                '<br>Days to approve =', mydf2$df.results$time_to_approval ,
                '<br><br><h4>Meta Data and Query </h4>') )
})
