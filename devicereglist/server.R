...
getreportid <- reactive({
  mydf <- getquery()
  tmp <- mydf$df.results
  id <- paste0( 'Application # = ', tmp$pma_number , tmp$k_number )
  if (is.null(id)){
    id = 'Missing Application Number'
  }
  return(id)
})
...
getfullquery <- reactive({
  ...
  myurl <- buildURL(v1, t1, limit=1, db='/device/', type='registrationlisting')
...
#  browser()
  myurl <- buildURL(v1, t1,  limit=1, skip=getskip(), db='/device/', type='registrationlisting')
  mydf <- fda_fetch_p(session, myurl)
  #   print('url')
  out <- c(df=mydf, url=myurl )
  return(out)
...
#OVERVIEW**********************
output$overviewtabletitle <- renderText({  
  s  <- paste('<h4>', getreportid(), '<br><br>Product</h4>' )
      return( s )
  })
...
output$ownertabletitle <- renderText({  
  s  <- paste('<h4>Owner</h4>' )
  return( s )
})

output$owner <- renderTable({  
  mydf <- getquery()
  mydf <- mydf$df.results$products[[1]]
#  browser()
...
output$registration <- renderTable({  
  mydf <- getquery()
#  browser()
  mydf <- mydf$df.results$registration
 # myvars <- gsub( 'device.','',  getdeviceidentificationvars(), fixed=TRUE)
  
  myvars <- getregistrationvars()
  return(extractcols(mydf, myvars))
})


output$registrationtabletitle <- renderText({  
  s  <- paste('<h4>Registration Variables</h4>' )
  return( s )
})
...
output$devicemodel <- renderTable({  
  mydf <- getquery()
    mydf <- mydf$df.results$products[[1]]$openfda
    myvars <- names(mydf)
 #   browser()
  return(extractcols(mydf, myvars))
})
...
output$mdrtabletitle <- renderText({  
  s  <- paste('<h4>', getreportid(), ' <br><br>Report Text</h4>' )
  return( s )
})
...
output$openfdatabletitle <- renderText({  
  s  <- paste('<h4>', getreportid(), '<br><br>OpenFDA variable</h4>') 
  return( s )
})
...
output$sourcetabletitle <- renderText({  
  s  <- paste('<h4>', getreportid(), '<br><br>Source</h4>' )
  return( s )
})
...
output$user_fitabletitle <- renderText({  
  s  <- paste('<h4>', getreportid(), '<br><br>User Facility/Importer</h4>' )
  return( s )
})
...
output$user_dmtabletitle <- renderText({  
  s  <- paste('<h4>', getreportid(), '<br><br>Device Manufacturere</h4>' )
  return( s )
})
...
output$suspecttabletitle <- renderText({  
  s  <- paste('<h4>', getreportid(), '<br><br>Suspect device manufacturer</h4>' )
  return( s )
})
...
output$keystabletitle <- renderText({  
  s  <- paste('<h4>', getreportid(), '<br><br>Keys and flags</h4>' )
  return( s )
})
...
output$othertabletitle <- renderText({  
  s  <- paste('<h4>', getreportid(), '<br><br>Patient</h4>' )
  return( s )
})
...
output$querytitle <- renderText({ 
  return( paste('<h4>', getreportid(), '<br><br>Meta Data and Query </h4>' ))
})
