...
getfullquery <- reactive({
  ...
  myurl <- buildURL(v1, t1, limit=1, db='/device/', type='pma')
...
getquery <- reactive({
  ...
  myurl <- buildURL(v1, t1,  limit=1, skip=getskip(), db='/device/', type='pma')
...
output$openfdatabletitle <- renderText({  
  mydf2 <- getquery()
  s  <- paste('<h4>PMA Number=', getreportid(), '</h4>',
...
output$querytitle <- renderText({ 
  mydf2 <- getquery()
  return( paste('<h4>PMA Number =', getreportid(), '</h4>',
#                '<br>Days to approve =', mydf2$df.results$time_to_approval ,
...
output$reportid <- renderUI({
  mydf2 <- getquery()
  p( paste('PMA Number=', getreportid() ) )
})
