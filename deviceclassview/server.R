...
getreportid <- reactive({
  ...
  tmp <- mydf$df.results
  id <- tmp$device_name
  id <- paste(id, '<br><br>Definition:', tmp$definition  )
  if (is.null(id)){
    id = 'Missing Event ID '
  }
  return(id)
...
getfullquery <- reactive({
  if ( input$t1==''  & input$t2 == '' & input$t3 == '' ){
    v1 = '_exists_'
    t1 = 'openfda.registration_number'
    v2 <- ''
    t2 <- ''
    v3 <- ''
    t3 <- ''
  ...
  myurl <- buildURL(v1, t1, limit=1, db='/device/', type='classification')
...
getquery <- reactive({
  ...
  myurl <- buildURL(v1, t1,  limit=1, skip=getskip(), db='/device/', type='classification')
  myurl <- gsub('Any Variable:', "", myurl, fixed=TRUE)
  mydf <- fda_fetch_p(session, myurl)
#       s <- ( names(mydf$results) )
#       write( s, file='')
  out <- c(df=mydf, url=myurl )
  return(out)
})    
...
output$table1title <- renderText({   
  mydf2 <- getquery()
  s  <- paste('<h4>Device Name =', getreportid(),
              '<br><br>Classification Fields', 
              '</h4>') 
  return( s )
})
...
#   mydf[, 'k_numbers'] <- listtostring(mydf[, 'k_numbers'])
#   mydf[, 'pma_numbers'] <- listtostring(mydf[, 'pma_numbers'])
  myvars <- names(mydf)
...
output$table2title <- renderText({   
  mydf2 <- getquery()
  s  <- paste('<h4>OpenFDA Fields</h4>') 
  return( s )
})
output$table2 <- renderTable({  
  mydf <- getquery()$df.results$openfda 
  curvars <- names(mydf)
#   if ( 'k_numbers' %in% curvars)
#   {
#     mydf[, 'k_numbers'] <- listtostring(mydf[, 'k_numbers'])
#     }
#   
#   if ( 'pma_numbers' %in% curvars)
#   {
#     mydf[, 'pma_numbers'] <- listtostring(mydf[, 'pma_numbers'])
#   }
#   if ( 'registration_number' %in% curvars)
#   {
#     mydf[, 'registration_number'] <- listtostring(mydf[, 'registration_number'])
#     }
#   
#   if ( 'fei_number' %in% curvars)
#   {
#     mydf[, 'fei_number'] <- listtostring(mydf[, 'fei_number'])
#     }
#   s <- ( names(mydf) )
#   write( s, file='')
  myvars <- names(mydf)
...
output$reportid <- renderUI({
  mydf2 <- getquery()
  p( paste('Device Name =', getreportid() ) )
})
